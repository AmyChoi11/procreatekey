...
int button3Function = 3; // Default: Undo
...
class ConfigCallbacks : public BLECharacteristicCallbacks {
  void onRead(BLECharacteristic* pChar) {
    // Send current configuration when read
  ...
  void onWrite(BLECharacteristic* pChar) {
    ...
    if (doc.containsKey("buttons")) {
      ...
      if (buttons.containsKey("button1")) newB1 = (int)buttons["button1"] | 0;
      if (buttons.containsKey("button2")) newB2 = (int)buttons["button2"] | 0;
      if (buttons.containsKey("button3")) newB3 = (int)buttons["button3"] | 0;

      Serial.printf("About to save prefs - before: b1=%d b2=%d b3=%d\n", button1Function, button2Function, button3Function);
      Serial.printf("New values received - b1=%d b2=%d b3=%d\n", newB1, newB2, newB3);

      // Validate and clamp values
      auto clamp = [](int v) { if (v < 0) return 0; if (v > 4) return 4; return v; };
      newB1 = clamp(newB1);
      newB2 = clamp(newB2);
      newB3 = clamp(newB3);

      // Force write to preferences
      ...
      Serial.printf("Prefs saved: b1=%d b2=%d b3=%d\n", button1Function, button2Function, button3Function);
...
void setup() {
  ...
  Serial.printf("Button functions: b1=%d b2=%d b3=%d\n", button1Function, button2Function, button3Function);
}
